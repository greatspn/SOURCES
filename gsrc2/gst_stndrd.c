...
void decode_mark(unsigned char *Mp,
                 unsigned short *dmp) {
...
int
evaluate_next(unsigned short *mrk) {
    int val, val2;
...
int
evaluate_cond(struct Cond_descr *cond, unsigned short *mrk) {
    int val, val2;
...
// #endif
            ...
            if (invoked_from_gui()) // write the name of the measure
                putc(cc, stderr);
        }
// #ifdef DEBUG
//         fprintf(stderr, "\n\n");
