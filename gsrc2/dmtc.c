/*
 *  Torino, April 22, 1988
 *  program: dmtc.c
 *  purpose: Reduction of a DSPN and construction of the EMC.
...
load_subnet() {
    ...
    for (i = subnet_num ; i-- ; smp1++) {
...
#ifdef DEBUG
        printf("            type=%d, t_num=%d, p_num=%d, shared_num=%d, equiv_s=%d\n", type, t_num, p_num, shared_num, equiv_s);
#endif
        smp1->type = (ROWT)type;
        ...
        *cp = (ROWT)0;
        ...
        smp1->det_trans = cp = (ROWP)ecalloc(shared_num + 2, ROWT_SIZE);
        *cp++ = shared_num;
