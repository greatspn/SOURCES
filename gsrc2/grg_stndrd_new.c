 ...
 *  purpose: TRG construction of a GSPN without net-dependent files compilation.
...
int pri_0, first_immediate, last_timed, transnump1;
...
do_fire(np, change_p, oldmrk, newmrk)
unsigned np;
...
union Link_descr *
        tst_ins_array(nn, Mp, nb, byte_p, val_p, father_p, leaf_p)
        unsigned nn;
unsigned char *Mp;
unsigned nb;
...
union Link_descr *
        tst_ins_tree(nn, Mp, nb, byte_p, val_p, father_p, leaf_p)
        unsigned nn;
unsigned char *Mp;
unsigned nb;
struct Byte_descr *byte_p;
...
timed_list(old_e, new_e, nt, aep, tep, Mp, enlistp)
unsigned short *old_e, *new_e;
int nt;
unsigned short *aep;
unsigned short *tep;
unsigned char *Mp;
union List_descr *enlistp;
{
...
#ifdef DEBUGCALLS
    fprintf(stderr, "   Start of timed_list\n");
#endif
    for (oel = *old_e ; oel ; oel = *(++old_e)) {
#ifdef DEBUGtimed_list
    ...
    *(new_e++) = 0;
    ...
    noel = (noel >> 1);
#ifdef DEBUGtimed_list
    fprintf(stderr, " list of %d trans constructed\n", noel);
#endif
    *(cp) = (unsigned short)noel;
    while (*first)
        *(++cp) = *(first++);
    *(++cp) = (unsigned short)0;
#ifdef DEBUGCALLS
    fprintf(stderr, "   End of timed_list\n");
#endif
}


unsigned
imm_enabl(old_e, new_e, nt, aep, tep, Mp, stop)
unsigned short *old_e, *new_e;
int nt;
unsigned short *aep;
unsigned short *tep;
...
int stop;
{
...
#ifdef DEBUGCALLS
    fprintf(stderr, "   Start of imm_enab\n");
#endif
    if (!stop) {
        while (*old_e >= *new_e)
            ++old_e;
    ...
    for (oel = *old_e ; oel > last_timed ; oel = *(++old_e)) {
#ifdef DEBUGimm_enab
        fprintf(stderr, "     oel=%d, *aep=%d, *tep=%d\n", oel, (int)*aep, (int)*tep);
#endif
        while ((el = *aep) > oel) {
            ...
            if ((test_enabl(Mp, el))) {
                ...
                *(new_e++) = el;
                if (stop) {
                    *new_e = 0;
#ifdef DEBUGCALLS
                    fprintf(stderr, "   End of imm_enab TRUE\n");
#endif
                    return (TRUE);
                }
            }
    ...
    *(new_e++) = 0;
    if (stop) {
#ifdef DEBUGCALLS
        fprintf(stderr, "   End of imm_enab FALSE\n");
#endif
        return (FALSE);
    }
#ifdef DEBUGCALLS
    fprintf(stderr, "   End of imm_enab %d\n", new_e - first);
#endif
    return (new_e - first);
}
...
unsigned
tst_ins_mark(Mp, enlistpp, subnet, new_p)
unsigned char *Mp;
union List_descr **enlistpp;
unsigned subnet;
unsigned char *new_p;
{
    unsigned *mrkp, mrk;
...
init() {
    ...
    for (byte_pp = encoding, nn = 0 ; nn < sub_num ; nn++, byte_pp++) {
        ...
        *byte_pp = (struct Byte_descr *)ecalloc(jj, sizeof(struct Byte_descr));
        ...
        no_cod[nn] = jj;
        if (nn == 0) {
            ent_1 = (unsigned char *)ecalloc(no_cod, sizeof(unsigned char));
            ent_2 = (unsigned char *)ecalloc(no_cod, sizeof(unsigned char));
        }
    ...
    for (sp = stack + 1, usp = sp->el, nt = trans_num, trans_p = t_list + (nt - 1),
            pri = 0 ; nt > last_timed ; --trans_p, --nt) {
        unsigned mult;
#ifdef DEBUG
        fprintf(stderr, "    Start of enabling test for t%d (pri=%d\n", nt, pri);
#endif
...
#ifdef DEBUG
            fprintf(stderr, "    End of enabling test for t%d, enabled (pp=%d, pri=%d)\n",
                    nt, pp, pri);
#endif
        }
#ifdef DEBUG
    ...
    if (pri) { /* VANISHING INITIAL MARKING */
        ...
        *(usp++) = (unsigned short)0;
        ...
        unsigned char is_new;
        union List_descr *enlistp;
        for (sp = stack + 1, usp = sp->el, ; nt ; --trans_p, --nt) {
            unsigned mult;
#ifdef DEBUG
            fprintf(stderr, "    Start of enabling test for t%d (pri=%d\n", nt, pri);
#endif
...
#ifdef DEBUG
                fprintf(stderr, "    End of enabling test for t%d, %d-enabled\n",
                        nt, mult);
#endif
            }
#ifdef DEBUG
            else
                fprintf(stderr, "    End of enabling test for t%d, disabled\n",
                        nt);
#endif
        }
        *(usp++) = (unsigned short)0;
        nm = tst_ins_mark(*CM, &enlistp, 0, &is_new);
        store_compact((unsigned long)nm, rgfp);

        ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ??
        sp->el, (unsigned)(usp - sp->el),
    }
...
compute_vanpath(ntr, necs, nmark, is_tang, num_p, paths_pp)
unsigned ntr;
unsigned necs;
unsigned nmark;
int is_tang;
...
store_vanpath(nvan, paths_p, num)
unsigned nvan;
struct Vantrans_path *paths_p;
unsigned num;
{
    ...
    struct Change_descr *change_p, * chp;
    unsigned nochangeplaces;
    unsigned ii, nt, np;
    struct Trans_descr *trans_p;
...
recurs_firing(nm, pri, from, new_e, sp)
unsigned nm;
unsigned pri;
unsigned from;
unsigned short *new_e;
struct Stack_el *sp;
{
    ...
    unsigned different = (sp->sub_num != (sp - 1)->sub_num);
    ...
    while (*new_e) {
        ...
        to_m = fire_trans(sp);
        is_tang = (to_m <= toptan);
        compute_vanpath(*new_e, sp->ecs_num, to_m, different || is_tang,
                        &num, &to_tang_paths);
        new_e += 2;
    }
...
unsigned
immediate_firing(newmrk, pri, new_e, firsten, stckp1)
unsigned char *newmrk;
unsigned pri;
unsigned short *new_e;
struct Trans_descr *firsten;
...
struct Stack_el *stckp;
{
    ...
    fprintf(stderr, "    lasvan=%d, topvan=%d, lastan=%d, toptan=%d\n",
            lasvan, topvan, lastan, toptan);
    fprintf(stderr, "  End of fire_trans\n");
#endif
    return (nm);
}


fire_subnet(subn, oldmrk, firsten, addl, stckp)
int subn;
unsigned char *oldmrk;
struct Trans_descr *firsten;
unsigned *addl;
...
{
    int is_new;
    unsigned short *new_e = stckp->el;
    unsigned char *cmark = CM[subn];
    int nextsbn, nt;

    transcode_mark(oldmrk, 0, cmark, subn);
    stckp->nm = nm = tst_ins_mark(cmark, new_e, 0, subn, &(is_new));
    stckp->sub_num = subn;
    if ((stckp->is_new = is_new)) {
        *(new_e++) = *addl;
        while ((nt = *++addl) && (nextsbn = t_list[nt - 1].nosub) == subn) {
            if (test_enabl(oldmrk, nt))
                *(new_e++) = nt;
        }
        *new_e = 0;
        fire_imm(firsten, stckp);
    }
    else {
        while ((nt = nextsbn = *++addl) &&
                (nextsbn = (firsten = (t_list + (nt - 1)))->nosub)) == subn
            );
        }
    if ((subn = nextsbn)) {
        for (subn = 0 ; nt > last_timed ;
                nt = *++addl) {
            if (test_enabl(oldmrk, nt) {
            firsten = t_list + (nt - 1);
                subn = firsten->nosub;
    ...
    if (((unsigned)(stckp1 - stack)) >= MAX_STACK) {
        fprintf(stderr, "Sorry, stack overflow.");
        fprintf(stderr, " Increase constant MAX_STACK\n");
        (*terminate)();
    }
    fire_subnet(subn, oldmrk, firsten, addl, stckp1);
    else { /* REACHED TANGIBLE SET */
    }
}
...
char **envp;
{
    ...
    for (lastan = 0, next_print = 50 ; lastan++ < toptan ;) {
        ...
        while (no_trans--) {
            ...
            do_fire((unsigned)(firsten->nochangeplaces),
                    firsten->change_places,
                    oldmrk, newmrk);
#ifdef DEBUG
            {
                int ii = (*no_cod); unsigned char *cp = newmrk;
            ...
            fprintf(stderr, "\n");
#endif
            if ((subn = (*(new_e = firsten->add_list) > last_timed))) {
                for (subn = 0 ; (uu = *new_e) > last_timed ;
                        ++new_e) {
                    if (test_enabl(newmrk, uu) {
                    firsten = t_list + (uu - 1);
                        subn = firsten->nosub;
                        break;
                    }
...
#endif
                store_compact((unsigned long)0, rgfp);
                fire_subnet(subn, newmrk, firsten, new_e, &zero, stckp1);
                /* END VANISHING PATHS */ store_compact((unsigned long)0, rgfp);
            }
            else { /* TANGIBLE */
                unsigned char is_new;
                union List_descr *enlistp
                        nm = tst_ins_mark(newmrk, &enlistp, 0, &is_new);
                if (is_new) {
#ifdef DEBUG
                    fprintf(stderr, "    new tangible %d\n", (int)nm);
#endif
                    timed_list(old_e, stckp1->el, new_e, firsten->tst_list,
                               enlistp);
                }
#ifdef DEBUG
                else
                    fprintf(stderr, "    old tangible %d\n", (int)nm);
#endif
                store_compact((unsigned long)nm, rgfp);
            }
        }
