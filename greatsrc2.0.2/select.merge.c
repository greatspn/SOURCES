...
extern int merging;
extern struct net_object *merge_netobj;
...
int x, y;
{
    ...
    */
    ...
    for (temp_pl = NULL, place = sel_netobj->places;
            place != NULL;)
        if (IsLayerListVisible(place->layer)
                && is_in_move(place->center)) {
            if (temp_pl == NULL)
                sel_netobj->places = place->next;
            else
            ...
            for (arc = sel_netobj->arcs, temp_ar = NULL; arc != NULL;)
                if (arc->place == place) {
                    if (temp_ar == NULL)
                        sel_netobj->arcs = arc->next;
                    else
    ...
    for (temp_mp = NULL, mpar = sel_netobj->mpars;
            mpar != NULL;)
        if (IsLayerListVisible(mpar->layer) && is_in_move(mpar->center)) {
            if (temp_mp == NULL)
                sel_netobj->mpars = mpar->next;
            else
    ...
    for (temp_rp = NULL, rpar = sel_netobj->rpars;
            rpar != NULL;)
        if (IsLayerListVisible(rpar->layer) && is_in_move(rpar->center)) {
            if (temp_rp == NULL)
                sel_netobj->rpars = rpar->next;
            else
    ...
    for (temp_tr = NULL, trans = sel_netobj->trans; trans != NULL;)
        if (IsLayerListVisible(trans->layer)
                && is_in_move(trans->center)) {
            if (temp_tr == NULL)
                sel_netobj->trans = trans->next;
            else
    ...
    for (group = sel_netobj->groups; group != NULL; group = group->next)
    ...
    for (temp_re = NULL, res = sel_netobj->results;
            res != NULL;)
        if (is_in_move(res->center)) {
            if (temp_re == NULL)
                sel_netobj->results = res->next;
            else
    ...
    for (temp_lisp = NULL, lisp = sel_netobj->lisps;
            lisp != NULL;)
        if (is_in_move(lisp->center)) {
            if (temp_lisp == NULL)
                sel_netobj->lisps = lisp->next;
            else
...
XButtonEvent *ie;
{
    ...
    struct coordinate *point, *temp_po;
    struct group_object *group;
...
limit_selected(int disp) {
    ...
    for (result = selected.results; result != NULL; result = result->next) {
        ...
        w = 7 * strlen(result->tag);
        if (newx < select_left)
            select_left = newx;
        if (newx + w > select_width)
            select_width = newx + w;
        if (newy < select_top)
            select_top = newy;
        if (newy > select_height)
...
static void
repaint_selectedCB(w, client_data, call_data)
Widget w;
...
my_get_pixmap(pxpp, ds, xc, yc, w, h)
Pixmap         *pxpp;
