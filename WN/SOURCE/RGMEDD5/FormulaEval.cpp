...
const std::vector<ref_ptr<Formula>> *p_greatspn_atomic_propositions = nullptr;
...
ref_ptr<BaseFormula> parse_and_evaluate_formula(Context& ctx, const std::string& formula, result_t *out_result) {
    ...
    if(parsedFrm != NULL) {
        ...
        if (parsedFrm->isIntFormula()) { 
            IntLiteral* intFrm = dynamic_cast<IntLiteral*>(parsedFrm.get());
            *out_result = result_t{(ssize_t)intFrm->getConstant()};
        }
        else { // boolean formula
            Formula* boolFrm = dynamic_cast<Formula*>(parsedFrm.get());
                   ...
                   const std::vector<size_t>& spot_ap_to_greatspn_ap_index)
{
    ...
    ref_ptr<Formula> stateFormula = dynamic_ptr_cast<Formula>(parse_formula(expr));
...
void model_check_query(Context& ctx, const ctl_query_t& query, int sem_id, bool *set_regr_failed) 
{
    ...
    ref_ptr<BaseFormula> formula = parse_and_evaluate_formula(ctx, query.line, &result);
    ...
    if (formula && formula->isBoolFormula() && (query.lang!=Language::LTL) && compute_card) {
        dd_edge dd(dynamic_cast<Formula*>(formula.get())->getMDD(ctx));
