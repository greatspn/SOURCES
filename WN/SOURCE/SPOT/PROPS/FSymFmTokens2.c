...
void
Afficher(TYPE_P **PM, TYPE_P NbElPM, int *ClArr, TYPE NbCl) {
    TYPE j, cl, ind, v;
    TYPE_P val;

    for (cl = 0; cl < NbCl; cl++)
        for (j = 0; j < NbElPM[ClArr[cl]]; j++) {
            printf("\n La partition :%d de la classe %s  : ", j,
                   tabc[ClArr[cl]].col_name);
...
CLASS_P
AllocateCl(int size, CLASS_P ptr) {
    CLASS_P pt;
...
void
FreeArrOfData(TYPE_P *DM, TYPE NbTokens) {
    TYPE j;
...
void
FreePartMAt(TYPE_P **PM, TYPE_P NbElPM) {
...
void
TreatOrdredClasses(TYPE_P **PM, TYPE_P NbElPM) {
    int cl, sb, c;
...
void
FreeClasses(CLASS_P Cls, TYPE NbCL) {
    TYPE i;
    for (i = 0; i < NbCL; i++)
        free(Cls[i].Set);
    free(Cls);
...
void
InitArray(TYPE_P *DM, TYPE NbTokens) {
    TYPE i;
...
int
GetIndObj(char *id, char *** MTCL, int *Cl) {
    int cl, i;
...
char *
GetObjFrInd(int ind, char *** MTCL, int Cl) {
    return MTCL[Cl][ind];
}

int
GetStaticInd(char *id, STORE_STATICS_ARRAY ASYM_CONF, int Cl) {
    int sb;
    Obj_p ptr;
    ...
    for (cl = 0; cl < ncl; cl++) {
        ...
        for (i = 0; i < tabc[cl].sbc_num; i++) {
            ...
            if (tabc[cl].sbclist[i].obj_list == NULL)
                for (j = tabc[cl].sbclist[i].low;
                        j < tabc[cl].sbclist[i].low + tabc[cl].sbclist[i].card;
                        j++) {
                    tmp1 = strdup(tabc[cl].sbclist[i].obj_name);
                    bzero(temp2, str_size * sizeof(char));
                    sprintf(tmp2, "%d", j);
                    MTCL[cl][k] = (char *)calloc(strlen(tmp1) + strlen(tmp2) + 1,
                                                 sizeof(char));
                ...
                for (Obj_list = tabc[cl].sbclist[i].obj_list;
                        Obj_list != NULL;
...
TYPE_P **
GetMatRepOfClParts(char *** MTCL, TYPE_P NbElPM) {
    const int SIZE = 256;
    int cl, j, i, CL;
    Obj_p Obj_list;
    ...
    for (cl = 0; cl < ncl; cl++)
        for (i = 0; i < tabc[cl].sbc_num; i++) {
            Parts_Mat[cl] = (TYPE_P *)realloc(Parts_Mat[cl],
                                              (NbElPM[cl] + 1) * sizeof(TYPE_P));
            ...
            if (tabc[cl].sbclist[i].obj_list == NULL)
                ...
                for (Obj_list = tabc[cl].sbclist[i].obj_list, j = 1;
                        Obj_list != NULL; Obj_list = Obj_list->next, j++)
...
PART_MAT_P
GetMatRepOfTransDom(char *** MTCL, Token_Domains dom) {
...
PART_MAT_P
GetMatRepOfPartTransDom(char *** MTCL, int pcl) {
...
CLASS_P
InitArrOfCl(TYPE_P Cls, TYPE size) {
    TYPE indx, i;
    CLASS_P ptr = NULL;
...
TYPE_P *
InitArrOfData(char *** MTCL,
              CToken_p pToken, TYPE_P NbTokens,
              TYPE_P Cls, TYPE size) {
...
void
DynDistOfObjects(TYPE_P **PM, TYPE *NbElPM,
...
void
SymRepOfToken(Token_Domains tk, Token_Domains *Simbtk, int **DynDis) {
    int i;
...
int
TestSymRepWtToken(Token_Domains Simbtk, Token_Domains tk, int **DynDis) {
    int i;
    for (i = 0; i < tk.NbCl; i++)
...
TYPE_P
CvtoBitArr(TYPE_P *DM, TYPE NbTokens, TYPE Cl) {
    TYPE_P temp = NULL;
    TYPE i;
...
TYPE
FindPosInPm(TYPE_P el, TYPE_P **PM,
            TYPE_P NbElPM, CLASS_P ClArr, TYPE CL) {
    TYPE j;
    ...
    PM[ClArr[CL].Cl] = (TYPE_P *) realloc(PM[ClArr[CL].Cl],
                                          (NbElPM[ClArr[CL].Cl] + 1) *
                                          sizeof(TYPE_P));
...
TYPE
CompTokens(TYPE ref, TYPE toCmp, TYPE Cur,
           TYPE_P *DM, TYPE NbCL) {
    int i;
    for (i = 0; i < Cur; i++)
...
void
FindNonDisjParts(CLASS_P ClArr, TYPE NbClas,
                 TYPE_P *DM, TYPE NbTokens,
...
void
FindDisjParts(CLASS_P ClArr, TYPE NbClas,
              TYPE_P **PM, TYPE_P NbElPM) {
    ...
    for (i = 0; i < NbClas; i++) {
        ...
        for (v = 1; v <= BIT_TYPE * MAX_IND; v++)
            if (Val(ClArr[i].Set, v) != 0) {
                ...
                if (!flag) {
                    NewArr = (INTERS *)realloc(NewArr,
                                               (necl + 1) * sizeof(INTERS));
                    NewArr[necl].Set = var;
...
PART_MAT_P
FindDisjParts3(PART_MAT_P PMS, TYPE NbElPMs) {
    TYPE i, j, k;
    int *ClArr = NULL;
    TYPE NbCls = 0;
    ...
    for (i = 0 ; i < NbElPMs ; i++)
        for (j = 0 ; j < ncl ; j++)
            for (k = 0 ; k < PMS[i].NbElPM[j] ; k++) {
                PMR->PART_MAT[j] = (TYPE_P *)realloc(PMR->PART_MAT[j],
                                                     (PMR->NbElPM[j] + 1) * sizeof(TYPE_P));
                PMR->PART_MAT[j][PMR->NbElPM[j]] = (TYPE_P) AllocateBV();
                memcpy(PMR->PART_MAT[j][PMR->NbElPM[j]],
                       PMS[i].PART_MAT[j][k],
                       MAX_IND * sizeof(TYPE));
                PMR->NbElPM[j]++;
            }
    ...
    PART_MAT_P pm = new_part(&(PMR->PART_MAT), &(PMR->NbElPM), MTCL);
...
TYPE_P **
FindDisjParts2(int *ClArr, TYPE NbClas,
               TYPE_P **PMF, TYPE_P NbElPMF,
               TYPE_P **PMM, TYPE_P NbElPMM) {

    int i, j;
    CLASS_P CLAS = InitArrOfCl(ClArr, NbClas);

    for (i = 0; i < NbClas; i++) {
        PMM[ClArr[i]] = (TYPE_P *)realloc(PMM[ClArr[i]],
...
TYPE_P **
ComputeLocalPartitions(char *** MTCL, TYPE_P Cls, TYPE NbCLs,
                       CToken_p pToken, TYPE_P *NbElPM) {
