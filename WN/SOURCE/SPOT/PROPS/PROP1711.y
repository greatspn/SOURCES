...
PROJ            :  {TD.NbCl=0;TD.Token=NULL;TD.Domain=NULL; }
			...
			int i;
			  ...
			  {
			    printf("\n '%s' n'est pas une place valide du réseau vérifié \n", $6);
			    return(1);
			  }
                        if(TD.NbCl==1 && TD.Token[0]==ELIMINATE) 
			  {
			    printf("\n La décolorisation n'est pas autorisée pour la place '%s'  \n ",$6);
			    return(1);
			  }
...
CONST           : CONST OR CONST {   int i; 
				       ...
				       {
                                         printf("\n Une ou plusieurs formules expriment une incompatibilité entre les domaines de couleurs des jetons : \n ");
			                 return(1);  
				       }
                  ...
                  IDENT LT VARS GT {
				     ...
				     if($2==0)
				       {
					 printf("\n La valeur '0' n'est pas une multiplicité valide pour un jeton \n");
					 return(1);
                                       }
				     $$->mult=$2;
				     ($$->tok_dom).Token=TD.Token;     
...
FORMULES        : ID ':' ATOMIC {   TreeNode_p pt1=$3;
					 ...
					 {
					   printf("\n La propostion atomique '%s' est doublement déclarée  \n",$1);
					   return(1);
					 }
