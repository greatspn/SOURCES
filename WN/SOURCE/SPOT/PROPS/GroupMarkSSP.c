...
void InitCurTreattemp(Tree_Esrg_p EventArr, STORE_STATICS_ARRAY STORED_CONF, STORE_STATICS_ARRAY SYM_CONF) {
    ...
    while (EventArr) {
        ...
        GET_TEMP_EVENTUALITIE_FROM_FILE(EventArr->marking->marking_as_string, EventArr->marking->length, TEMP_EVENT);
...
void InitCurTreat(Tree_Esrg_p EventArr, STORE_STATICS_ARRAY STORED_CONF, STORE_STATICS_ARRAY SYM_CONF) {
    int i, j, k, s, max_card = get_max_cardinality();
    ...
    while (EventArr) {
        ...
        GET_EVENTUALITIE_FROM_FILE(EventArr->marking->marking_as_string, EventArr->marking->length);
        COPY_CACHE(EventArr->marking->length);
        STRING_TO_EVENTUALITIE(SYM_CONF);
...
int Valid_Matrix_cl(int **mat1, int **mat2, int NbSbc, int NbDsc, int first, int second, int cl) {
    int i;
...
int Valid_Matrix(int *** mat1, int *** mat2, int *NbSbc, int *NbDsc, int first , int second, int cl, int ncl) {
    int i;

    for (i = 0; i < ncl; i++)
        if (i == cl) {
            if (!Valid_Matrix_cl(mat1[i], mat2[i], NbSbc[i], NbDsc[i], first, second, cl)) return false;
        }
        else if (!Valid_Matrix_cl(mat1[i], mat2[i], NbSbc[i], NbDsc[i], -1, -1 , cl)) return false;
...
void GetEquationSystem(int **mat, int *DSC, int *group, int Nbgroup, int first, int second, int cl) {
    int j;

    STATICS[0][0][0] = Get_Card(group, first, cl);
    STATICS[0][0][1] = Get_Card(group, second, cl);
    for (j = 0; j < DSC[cl]; j++) DYNAMIC[0][0][j] = GetSecondMember(mat, j, CONF[cl].sbc_num, first, second);
    NB_DSC[0][0] = DSC[cl];
    NB_SSBC[0][0] = 2;


}
int NB_SOLUTIONS(int **mat, int *DSC, int *group, int Nbgroup, int first, int second, int cl) {
...
void  LIBERER_ELEM(MarkAsEventp ptr , int *ord, float *rate) {
    int cl, sb;
    MarkAsEventp next;
    //  int ord=0;
        ...
        for (i = 0; i < NbCurList; i++)
            if (CurList[i]) {
                ...
                for (j = i + 1; j < NbCurList; j++)
                    if (CurList[j])
                        if (Valid_Matrix(CurList[i]->Event, CurList[j]->Event, SSC, DSC, I, J, cl, ncl)) {
                            (*ptr) = CurList[j];
                            CurList[j]->next = NULL;
                            ptr = &(CurList[j]->next);
                            CurList[j] = NULL;
                ...
                if (NB_SOL == NB_EXIST) {
                    ...
                    LIBERER_ELEM(CurList[i]->next, &(CurList[i]->ord), &(CurList[i]->rate));
                    CurList[i]->next = NULL;
                    (*Agr) = CurList[i];
                    Agr = &(CurList[i]->next);
                }
...
int NEXT_INDEX(int ind) {
    ...
    if (k >= 0)
        ...
        return -1;
    /* for(k=ind+1;k<CONF[CLAS].sbc_num && !Index[k];k++); */
    /*    // if(k!=CONF[CLAS].sbc_num) */
    /*    if(k<CONF[CLAS].sbc_num) */
    /*      return k; */
    /*    else */
    /*      return -1; */
    ...
    if ((j = NEXT_INDEX(ToGroup)) != -1) {
        ...
        if ((j = NEXT_INDEX(i)) != -1) {
            ...
            if (NEXT_CLASS(CLAS) != -1) {
                if (!flag)
                    (*ptr) = New_Element(CONF[CLAS + 1].sbc_num - 1, CONF[CLAS + 1].sbc_num - 2, CLAS + 1, false);
                else {
...
void GROUP_MARKINGS(Tree_Esrg_p EventArr, STORE_STATICS_ARRAY ASYM_CONF, STORE_STATICS_ARRAY SYM_CONF, char *** MTCL) {
    int i;
    ResultList = NULL;
    NbResList = 0;
...
void TEMP_GROUP_MARKINGS(Tree_Esrg_p EventArr, STORE_STATICS_ARRAY ASYM_CONF, STORE_STATICS_ARRAY SYM_CONF, char *** MTCL) {
    int i;
    ResultList = NULL;
    NbResList = 0;
...
/* void GROUP_MARKINGS_MC(Tree_Esrg_p EventArr,STORE_STATICS_ARRAY ASYM_CONF,STORE_STATICS_ARRAY SYM_CONF, char*** MTCL,int gr ) */
/* { */
/*   int i;  */
/*   ResultList=NULL; */
/*   NbResList=0; */
