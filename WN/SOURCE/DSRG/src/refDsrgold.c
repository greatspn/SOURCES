...
load_sym_rep(FILE *fd, int pos,
    ...
    // read the three first params. from
    // the ".off_sr" file.
    fseek(fd, 0, SEEK_SET);
    fread(CACHE_STRING_ESRG, 1, 100, fd);
    LP_ESRG = CACHE_STRING_ESRG;

    // main loop to read params. until the "pos"
    // position.
    for (i = 0; i <= pos; i++) {
        ...
        ps = ps + (LP_ESRG - CACHE_STRING_ESRG);
        fseek(fd, ps, SEEK_SET);
        fread(CACHE_STRING_ESRG, 1, 100, fd);
        LP_ESRG = CACHE_STRING_ESRG;
    }
...
load_event(FILE *fd,
    ...
    // read the first three params.
    // from the ".off_ev" file.
    fseek(fd, 0, SEEK_SET);
    fread(CACHE_STRING_ESRG, 1, 100, fd);
    LP_ESRG = CACHE_STRING_ESRG;

    //main loop, used to load the correct
    //three params. of an event.
    for (i = 0; i < pos; i++) {
...
load_tmp_symb_marking(FILE *fsr,
    ...
    load_sym_rep(fsr, nd->sr , &mark_pos, &llength,  &ld_ptr);
...
int get_refinement(TYPE_P **PM_mark ,
                   TYPE_P NbElPM_mark,
                   TYPE_P **PM , TYPE_P NbElPM,
...
get_refined_markings(FILE *fsr,
    ...
    load_event(fev, ev_pos, &sr,
               &llength, &ld_ptr, &PM, &NB_PM);
...
pDtmcN
grouping_and_store(FILE *fsr, FILE *fev, pDtmcN nd, pDtmcN *idx) {
...
Update_in_nodes(FILE *fsr,
    ...
    return grouping_and_store(fsr, fev, li, idx);
}
...
void
treat_loop(pDtmcN split, pDtmcN *idx) {
...
ref_algorithm(FILE *fsr,
    ...
    while (li) {
        ...
        split = Update_in_nodes(fsr, fev, li, idx);
        pDtmcN hdi = li->nxt_inc;

        if (SelfLoop) {
            treat_loop(split, idx);
        }

        if (Head == li) Head = split;
        pDtmcN hd = split;

        while (split) {
            get_ref_with_compare(fsr, fev, split, 1);
            hdi = Update_out_nodes(fsr, fev, split, idx, hdi);
            free_temp_cur(split);
...
load_symbolic_marking(FILE *STD,
    ...
    load_event(fev, ev_pos, &sr, &llength, &ld_ptr, &PM, &NB_PM);
    ...
    GROUPING_ALL_STATICS(MERG_ALL, tabc, num);
    /****************************************************************/
    load_sym_rep(fsr, sr , &mark_pos, &llength,  &ld_ptr);
...
void raff_usage(void) {
    ...
    fprintf(stdout, "\n\n-------------------------------------------------------\n");
    fprintf(stdout, "USE : raff_dsrg netname [-p,-a] (-p to store the refined DTMC) \n");
    fprintf(stdout, "----------------------------------------------------------\n\n");
