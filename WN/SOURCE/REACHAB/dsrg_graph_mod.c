...
int print_state(const State s, char **st) {
    ...
    StaticConf = get_tabc(s->part, MTCL);
...
extern int initial_state(pState M0) {
    ...
    if (! initial_marking) {
        ...
        StrPrt *loc_part =  alloc_StrPrt();
        loc_part->size = GetStringRepFromTabc(loc_part->part, MTCL);
        initial_statics = get_tabc(loc_part , MTCL);
        ...
        reached_marking->Head_Next_Event->loc_part = loc_part;
...
void TEST_AND_ADD_MC(MarkSym **tab_sym, int *Nbtabsym, int ord_class, int ord_event,
...
void d_srg_enabling_and_guards(State s) {
    ...
    s_StaticConf = get_tabc(s->loc_part, MTCL);
...
void mc_dsrg_insert_tree(int *** event, float rate, int ord, STORE_STATICS_ARRAY asym_static_conf,
    ...
    while (ptr) {
        PART_MAT_P   pm_ptr = GetMatRepFromString(ptr->loc_part->part, MTCL);
        PART_MAT_P   pm_par = GetMatRepFromString(loc_part->part, MTCL);

        PART_MAT_P   PM = GetRefinedPartitions(pm_ptr->PART_MAT, pm_ptr->NbElPM, pm_par->PART_MAT, pm_par->NbElPM);
        ...
        get_refined_eventualities(pm_ptr->PART_MAT, pm_ptr->NbElPM,  PM->PART_MAT , PM->NbElPM,
                                  &event_arry_event_inclued_in  , &size_arry_event_inclued_in);
        INIT_RESULT_STRUCT(event);
        get_refined_eventualities(pm_par->PART_MAT, pm_par->NbElPM, PM->PART_MAT , PM->NbElPM,
        ...
        if (new_mark_inc_old_mark &&  old_mark_inc_new_mark) {
            FreePartMAt(pm_par->PART_MAT, pm_par->NbElPM);  free(pm_par);
            FreePartMAt(pm_ptr->PART_MAT, pm_ptr->NbElPM);  free(pm_ptr);
    ...
    if (last_ptr != NULL) {
        ...
        last_ptr->Head_Next_Event->loc_part = loc_part;
    ...
    if (tro == NEW_MARKING) {
        ...
        *flg = false;
        ...
        while (ptr) {
            ...
            PART_MAT_P   PM = GetRefinedPartitions(pm_ptr->PART_MAT, pm_ptr->NbElPM,
                                                   pm_par->PART_MAT, pm_par->NbElPM);
...
PART_MAT_P d_srg_asym_succ_arc(State s, MarkSym **TAB_SYM, int *NbTAB_SYM) {
    ...
    PART_MAT_P   pm = GetMatRepFromString(s->loc_part->part, MTCL);

    s_prop_Part[0].PART_MAT = pm->PART_MAT;
    s_prop_Part[0].NbElPM  = pm->NbElPM;
    free(pm);
    ...
    old_StaticConf = GetColorPartitionFromString(s->loc_part, MTCL);
...
int WeakLump_cond(Tree_Esrg_p EventArr, MarkSym TAB_SYM, STORE_STATICS_ARRAY s_prop_StaticConf, StrPrt *loc_part) {
    ...
    if (!ptr) {
        pre_mc_dsrg_insert_tree(first, first->input_tr, first->marking->ordinary,
                                s_prop_StaticConf, loc_part, &size);
...
void dsrg_graph() {
    ...
    do {
        ...
        if (d_srg_top->enabled_head) {
            ...
            s_prop_StaticConf = get_tabc(d_srg_top->part, MTCL)
                                trans = 0;
        }
        else {
