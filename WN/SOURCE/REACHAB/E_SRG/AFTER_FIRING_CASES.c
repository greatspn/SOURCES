...
void
FREE_TREE(Tree_Esrg_p *Head,
          Tree_Esrg_p To_free,
          Tree_Esrg_p Prec) {
    if ((*Head) == To_free)
        (*Head) = To_free->Head_Next_Event;
    else if (To_free->Head_Next_Event == NULL)
        Prec->Head_Next_Event = NULL;
    else
        Prec->Head_Next_Event = To_free->Head_Next_Event;

    TREE_EVENT_FREE(To_free);
}

void
FREE_EVENTUALITIES(Tree_Esrg_p Head) {
...
void
INIT_ARRAYS(TO_MERGEP *merg,
            int **num,
            int *** card,
            STORE_STATICS_ARRAY STORED_CONF); // Forward declaration

int
TEST_SYMETRIC_INITIAL(TO_MERGEP *merg) {
    int nb;
...
Tree_Esrg_p
TEST_EXISTENCE(Tree_Esrg_p  List_Events,
               Tree_Esrg_p *prec) {
...
int
_seach_event_marking(marking_struct *tab,
                     int tab_size,
                     char *s_string,
                     int s_size) {
...
void
_free_event_marking(marking_struct *tab,
                    int tab_size) {
    int i;
    for (i = 0; i < tab_size; i++)
        free(tab[i].marking);
...
void
init_tabtmp(marking_struct **tab,
            int *tab_size,
            Tree_Esrg_p ptr) {
    ...
    while (ptr) {
        ...
        (*tab) = realloc((*tab), ((*tab_size) + 1) *
                         sizeof(marking_struct));
        (*tab)[(*tab_size)].length =
...
TRAITEMENT_1() {
    ...
    if (_seach_event_marking(tabtmp, sizetmp,
        ...
        tabtmp = realloc(tabtmp, (sizetmp + 1) * sizeof(marking_struct));
        ...
        if (enabled_head) {
            ...
            event                    = MY_TREE_POP(FILE_POS_PTR,
                                                   LEGTH_OF_CACHE);
...
int
SYM_SAT_2_NOT_EXIST(Tree_p Reached_Marking,
...
TRAITEMENT_2() {
    ...
    if ((event = TEST_EXISTENCE(REACHED_MARKING->Head_Next_Event,
                                &prec)) == NULL) {
        enabled_head = NULL;
        ...
        if (_seach_event_marking(tabtmp, sizetmp,
            ...
            tabtmp = realloc(tabtmp, (sizetmp + 1) *
                             sizeof(marking_struct));
            tabtmp[sizetmp].length =
                _copy_cache_string(&tabtmp[sizetmp].marking);
            sizetmp++;
            ...
            if (enabled_head) {
                ...
                if (out_mc)
                    SR_EVENT_POS_STORE(OFFSET_EVENT,
                                       REACHED_MARKING, event);
            }

            COMPT_EVENT++;
        }

        // Re-load the symetric config
...
int
SYM_SAT_2_EXIST(Tree_p Reached_Marking,
                Tree_p Source_Marking,
                Event_p ev_p, TO_MERGEP *merg) {
    ...
    if (Reached_Marking->Marking_Type == NO_SATURED_SYM) {
        ...
        // Compute the enable instances
        // of the symetric transitions.
    ...
    if (Reached_Marking->Marking_Type != SATURED_SYM) {
        ...
        if ((Output = TEST_EXISTENCE(Reached_Marking->Head_Next_Event, &prec)) == NULL) {
            ...
            if (Reached_Marking->Num_Event == 1) {
                ...
                COMPT_SAT++;
                // Computation of the symetrical instances
                // for the symetric marking
                Reached_Marking->Marking_Type = MARKING_TYPE = SATURED_SYM;
            ...
            if ((MARKING_TYPE != SATURED_INS) ||
                    ((MARKING_TYPE == SATURED_INS) && (enabled_head))) {
                ...
                // If the marking is archeaved,
                // it must added to the stack.
                if (Reached_Marking->NOT_Treated == NULL)
                    Reached_Marking->NOT_Treated = New_event;
...
int ASYM_2_NOT_EXIST_SYM(Tree_p Reached_Marking,
    ...
    if (output_flag)
        Print_Ins_Arc(OUTPUT_FILE, ev_p->trans,
                      Eventual->marking->cont_tang,
