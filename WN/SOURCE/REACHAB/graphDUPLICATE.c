...
extern FILE *f_CVRS;
static void garbage_collectE(Result_p);
#endif
//MDWN
...
/* RETURN VALUE : */
/**************************************************************/
static void garbage_collectE(Result_p enabled_head) {
        ...
        if (tabt[tr].pri == marking_pri) {
            ...
            while (ev_p != NULL) {
                ...
                if ((cur_priority) || (!cur_priority) && ((mean_t *mean >= Threshold)) || (non_det)) {
                    ...
                    if (!IS_VANISHING(cur_priority))
                        insert_tree(&root, &h, f_mark, length, d_ptr);
                    else {
                        tro = NEW_MARKING;
                        tmp_marking = current_marking = reached_marking = treenode_pop(f_mark, length, d_ptr);
                    }

                    //garbage_collectE(enabled_head);
#else
            ...
            if (tabt[tr].pri == marking_pri) {
                ...
                while (ev_p != NULL) {
                    ...
                    if (!IS_VANISHING(cur_priority))
                        ...
                        tro = NEW_MARKING;
                        //free reached_marking
                        if ((tmp_marking != NULL) && (tmp_marking->marking != NULL) &&) {
                            free(tmp_marking->marking);
                            free(tmp_marking);
                        }
                        //free reached_marking
                        tmp_marking = current_marking = reached_marking = treenode_pop(f_mark, length, d_ptr);
