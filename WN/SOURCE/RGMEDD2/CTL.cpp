...
void CTLMDD::CTLinit() {
    ...
    *ins = new int[nvar + 1];
    for (int j = 1; j <= nvar; j++) {
    	#warning "Usare dont-care dont-change. Chiedere a Marco per il double-level."
        ins[0][j] = DOUBLELEVEL ? -2 : DONT_CARE;
    }
...
void PlaceTerm::createMTMDD() {
    ...
    int **m = ctl->getIns();
    float terminale;
    int variable = rsrg->convertPlaceToMDDLevel(place) + 1;
...
#warning "non dovrebbero mai comparire PLUS e MINUS, controllare"
    ...
    #warning "usare dont care/change"
    m[0][variable] = DOUBLELEVEL ? -2 : DONT_CARE;
    setMTMDD(tmp_mdd);
