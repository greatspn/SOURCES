...
int put_trans(netobj, trans, knd, num)
...
int             num;
{
    ...
    /* count input arcs */
    for (arc = trans->arcs, j = 0; arc != NULL; arc = arc->next)
...
struct trans_object *trans;
{
    ...
    for (arc_pr = trans->arcs; arc_pr != NULL; arc_pr = arc_pr->next)
        if (arc_pr->type == kind) {
