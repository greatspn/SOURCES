...
//         }
            ...
            *c_p = '\0';
            ...
            lisp->text = (char *)strdup(templisp);
            free(templisp);
            fgets(linebuf, LINEMAX, dfp);
        }
    }
...
#endif
    /* read layer names  */
    SetLayersNumber(nolay);
    for (ln = 1; ln <= nolay; ln++) {
        fgets(linebuf, LINEMAX, nfp);
        for (c_p = linebuf; *c_p != '\n' && *c_p != '\0'; c_p++);
        if (*c_p == '\n')
            *c_p = '\0';
...
int getarcs(netobj, kind, trans, noar)
struct net_object *netobj;
char            kind;
