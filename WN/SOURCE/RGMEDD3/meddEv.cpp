...
void RSRG::buildOtfRelation() {
    ...
    p_otf_unprimed_minterms = new int[npl + 1];
    p_otf_primed_minterms = new int[npl + 1];
...
struct dot_of_DD {
    ...
    void start_visit(const dd_edge& e) {
        ...
        const int max_levels = e.getLevel();
        nodes_per_lvl.resize(max_levels + 1);

        visit(e.getNode());
        dot << "}\n";

        if (write_level_labels) {
            // write places in a separate cluster
            dot << "  subgraph cluster2 { style=invis;\n  node [shape=none, fontsize=60, margin=\"0.5,0.1\"];\n";
            for (int p=0; p<npl; p++) {
                size_t lvl = rs->convertPlaceToMDDLevel(p) + 1;
                dot << "  P"<<lvl<<" [label=\""<<tabp[p].place_name<<"\"];\n";
                nodes_per_lvl[lvl] += " P";
                nodes_per_lvl[lvl] += std::to_string(lvl);
            }
            dot << "}\n";
        }
...
void RSRG::countNodesPerLevel(NodesPerLevelCounter& nplc, const dd_edge& dd) {
    ...
    nplc.nodesPerLvl.resize(npl, 0);
    nplc.singletoneNodesPerLvl.resize(npl, 0);
...
void RSRG::showExtendedIncidenceMatrix(bool show_saved_file) {
    ...
    for (int i=0; i<2; i++) {
        ...
        DD[i].footer = std::to_string(std::accumulate(nplc.nodesPerLvl.begin(), nplc.nodesPerLvl.end(), 0));
