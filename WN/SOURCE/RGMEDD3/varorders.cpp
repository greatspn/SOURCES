                         ...
                         flow_basis_metric_t& fbm) 
{
    ...
    const std::vector<PSemiflow> empty_psf;
    ...
    switch (sel.heuristics) {
	    ...
	    case VOC_PINV: {
            ...
            // std::vector<int> vo;
            // varorder_P_chaining(load_Psemiflows(), vo);

            // for (int i=0; i<npl; i++) {
            //     cout << net_to_mddLevel[i] << " " << vo[i] << endl;
            // }
        ...
        case VOC_FORCE:
            ...
            var_order_FORCE(sel.heuristics, net_to_mddLevel, net_to_mddLevel, empty_psf, sel.verbose);
            break;

        case VOC_FORCE_PINV:
            var_order_visit(VOC_BFS, net_to_mddLevel, 0);
            var_order_FORCE(sel.heuristics, net_to_mddLevel, net_to_mddLevel, load_Psemiflows(), sel.verbose);
            break;
	    ...
	    default:
	        throw rgmedd_exception("Internal error: Unknown variable order heuristics constant.\n");
    }
    ...
    if (g_print_varorder_metrics & !is_meta_heuristics) 
    {
        ...
        const std::vector<PSemiflow>& psf = load_Psemiflows();
        const int* bounds = load_bounds();
        std::vector<size_t> lw_psf(npl, 0);//, lw_psf_bnds;
        for (const PSemiflow& p : psf) {
            int lvlbot, lvltop;
            lvlbot = lvltop = net_to_mddLevel[p[0].place_no];
            for (auto& e : p) {
                lvlbot = min(lvlbot, net_to_mddLevel[e.place_no]);
                lvltop = max(lvltop, net_to_mddLevel[e.place_no]);
            }
                    ...
                    metric_t* out[NUM_METRICS])
{
    ...
    auto require_lw_psf = [&]() {
        if (lw_psf.empty()) {
            ...
            for (const PSemiflow& p : psf) {
                int lvlbot, lvltop;
                lvlbot = lvltop = net_to_level[p[0].place_no];
                for (auto& e : p) {
                    lvlbot = min(lvlbot, net_to_level[e.place_no]);
                    lvltop = max(lvltop, net_to_level[e.place_no]);
                }
                // compute the p-semiflow span
                for (int lvl = lvlbot; lvl <= lvltop; lvl++) {
                    lw_psf[lvl]++;
...
load_Psemiflows() {
    static std::vector<PSemiflow> net_psf; // Permanently stored P-semiflows
...
{
    static std::vector<PSemiflow> nupn_psf; // Permanently stored NuPN
...
bool load_flow_basis() {
    if (!s_basis_loaded) {
        ...
        s_have_basis = load_flows_from_file(pif, s_fb);
        if (s_have_basis) {
            for (auto&& row : s_fb)
                for (auto&& el : row)
...
bool load_pflows() {
    if (!s_pflows_loaded) {
        ...
        s_have_pflows = load_flows_from_file(pif, s_pflows);
    }
...
int measure_PSF(const std::vector<int> &varorder) {
    ...
    for (int p=0; p<psf.size(); p++) {
        if (psf[p].size() == 0)
            continue;
        int top = varorder[ psf[p][0].place_no ];
        int bot = top;
        for (int i=1; i<psf[p].size(); i++) {
            top = std::max(top, varorder[ psf[p][i].place_no ]);
            bot = std::min(bot, varorder[ psf[p][i].place_no ]);
        }
        PSF += (top - bot + 1);
    }
    return PSF;
...
void var_order_FORCE(const VariableOrderCriteria voc, std::vector<int> &out_order, 
                     const std::vector<int> &in_order, const std::vector<PSemiflow>& psf, 
                     bool verbose) 
{
...
double measure_FORCE_pts(const VariableOrderCriteria voc, const std::vector<int> &in_order,
                         const std::vector<PSemiflow>& psf) 
{
    std::vector<int> num_trns_of_place(npl);
    std::vector<double> cog(ntr);//, grade(npl);
    // std::vector<std::pair<double, int>> sorter(npl);
...
static int find_min(const std::vector<PSemiflow>& PSF);
static int find_max(const std::vector<PSemiflow>& PSF, int keep, std::vector<int> &checked_pinv);
...
void varorder_P_chaining(const std::vector<PSemiflow>& PSF, std::vector<int>& new_map_sort) {
    ...
    while (move) {
        ...
        if (keep_sec == -1) { // if intersection is empty, put in the vector only the places of the first p-invariant
            for (const PSemiflowEntry& e : PSF[keep]) {
                if (!selected_place[e.place_no]) { // else i store it in the vector
                    new_map_sort[cnt++] = e.place_no;
                    selected_place[e.place_no] = true;
                }
            }
        }
            ...
            for (const PSemiflowEntry& e : PSF[keep]) {
                ...
                for (const PSemiflowEntry& e2 : PSF[keep_sec]) {
                    if (e.place_no == e2.place_no) {
                        is_common = true;
                        continue; // common place
                    }
                }
                if (!is_common && !selected_place[e.place_no]) {
                    new_map_sort[cnt++] = e.place_no;
                    selected_place[e.place_no] = true;
                }
            }
            ...
            for (const PSemiflowEntry& e : PSF[keep]) {
                for (const PSemiflowEntry& e2 : PSF[keep_sec]) {
                    if (e.place_no == e2.place_no) {
                        if (!common_places_are_not_touched(not_touched, e.place_no)) { // if it isn't already in common places
                            if (!selected_place[e.place_no]) { // if it isn't on the new_map_sort
                                new_map_sort[cnt++] = e.place_no;
                                selected_place[e.place_no] = true;
                                not_touched[e.place_no] = e.place_no; // places which I am not allowed to touch
                            }
                            else { // if common places exists and I can touch it (isn't already in common )
                                int j;
                                for (j = 0; j < npl; j++) {
                                    if (new_map_sort[j] == e.place_no) // keep the position of new common place
                                        break;
                                }
                                // cout << "moving " << e.place_no << " from pos " << j << " to pos " << (cnt-1) << endl;
                                for (int tmp = j; tmp < cnt - 1; tmp++) {
                                    new_map_sort[tmp] = new_map_sort[tmp + 1]; // reorder the vector
                                }
                                new_map_sort[cnt - 1] = e.place_no;
                            }
                        }
                    }
                }
            ...
            for (const PSemiflowEntry& e2 : PSF[keep_sec]) {
                ...
                if (!is_common && !selected_place[e2.place_no]) {
                    new_map_sort[cnt++] = e2.place_no;
                    selected_place[e2.place_no] = true;
                }
            }
        }
...
static int find_min(const std::vector<PSemiflow>& PSF) {
    ...
    for (int i = 0; i < num_pinv; i++) {
        ...
        for (int j = i + 1; j < num_pinv; j++) {
            int k = 0;
            for (PSemiflowEntry e : PSF[j]) {
                // Search if e.place is in common with PSF[i]
                for (size_t h=0; h<PSF[i].size(); h++) {
                    if (PSF[i][h].place_no == e.place_no) {
                        k++;
                        break;
                    }
                }
