...
const std::vector<PSemiflow>& load_Psemiflows();
...
// typedef sparsevector<int, int> sparse_vector_t;
...
void var_order_FORCE(const VariableOrderCriteria voc, std::vector<int> &out_order, 
                     const std::vector<int> &in_order,  const std::vector<PSemiflow>& psf, bool verbose);
...
void var_order_gradient_P(const VariableOrderCriteria voc, const std::vector<PSemiflow>& psf, std::vector<int> &out_order);
