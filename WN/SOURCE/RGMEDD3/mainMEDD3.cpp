...
void usage(bool long_banner) 
{
    if (long_banner && !running_for_MCC()) {
        print_banner("");
        cout << "USE :  RGMEDD3  <netname> [options...]\n\n"
                "Options description:\n"
                ...
                "  -bfs-mono    Use BFS with monolithic NSF. \n"
    ...
    cout << "  Copyright (C) 1987-2018, University of Torino, Italy." << endl;
    if (!running_for_MCC()) {
...
#ifdef SIMULATION
# error "SIMULATION should not be enabled in RGMEDD3."
#endif

#ifdef LIBSPOT
# error "LIBSPOT should not be enabled in RGMEDD3."
#endif
...
// #endif
    ...
    for (ii = 2; ii < argc; ii++) {
        ...
        else if (0 == strcmp(argv[ii], "-multitest") && ii + 2 < argc) {
            // g_refine_annealing = true;
    ...
    if (!running_for_MCC()) {
        switch (initRsMethod) {
            ...
            case RSM_BFS_MONOLITHIC_COV:
                cout << "Using BFS with monolithic next-state-function." << endl;
                build_NSF = true;
                break;
            case RSM_SAT_MONOLITHIC:
                cout << "Using saturation with monolithic next-state-function." << endl;
                build_NSF = true;
                break;
            case RSM_SAT_PREGEN:
