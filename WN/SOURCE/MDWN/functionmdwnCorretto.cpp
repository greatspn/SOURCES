...
/***********************************************************************************/
	...
	if (buffer=="|")
		...
		struct NOOBJS noobjs=AddPlaceTransNum(out,buffer,CompAtt.size(),CompPas.size());
		unsigned int i=1;
		//mark
		string info;
		...
		int NumPlace=noobjs.NP+4*CompAtt.size()+2*CompPas.size();
...
{
if (Gl)
	out<<"Pr2Nd 1.000000e+00 0 0 "<<CompAtt.size()+CompPas.size()-1<<" 0 0 0 0 0 0 0 0\n";
else
	out<<"Pr2Nd 1.000000e+00 0 0 "<<CompAtt.size()+CompPas.size()<<" 0 0 0 0 0 0 0 0\n";
...
{
out<<"Nd2Pr 1.000000e+00 0 0 "<<CompAtt.size()<<" 0 0 0 0 0 0 0 0\n";
unsigned int step=CompAtt.size();
map <std::string,class component>::iterator iter;
iter=CompAtt.begin();
//input arc
...
//input arc
//output arc
step=CompAtt.size()+CompPas.size();
if (Gl) step=step-1;
