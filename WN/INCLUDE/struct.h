...
#endif
/***** Nested Units (NuPN) *****/
/* As described in : Nested-Unit Petri Nets: A Structural Means to Increase Efficiency 
 * and Scalability of Verification on Elementary Nets - Hubert Garavel 
 * and saved by PNML->net/def translator of Editor.jar */
typedef struct NESTED_UNIT {
    int id;
    struct NESTED_UNIT *parent;
    char *name;
    int num_subunits;
    struct NESTED_UNIT **subunits;
    int num_places;
    int *places;
} NESTED_UNIT;
extern NESTED_UNIT *nu_root; // hyerarchical tree of nested units
extern NESTED_UNIT **nu_array; // flat array of all the nested units
extern int num_nested_units; // number of nested units
/***** TABELLA DEI POSTI *****/
struct PLACES {
    char *place_name;
    char *algebra_tags;
...
#ifdef MDWN
    double uncertainty; // For uncertain MDPN
#endif
    // Extended general functions are stored in this field.
    // It is NULL if the transition has no general function assigned.
    char *general_function;
    //It is a pointer to the rate paramater table, it is used da PN2ODE
    int  rate_par_id;
} ;
...
#ifdef SWN
#ifdef SYMBOLIC
    double ordinary;	 /* marcature ordinarie corrispondenti	  */
#endif
#endif
};
