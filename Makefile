...
$(call search_library,GMP_LIB,/libgmpxx.*,"GMP library",-lgmp -lgmpxx)
ifdef HAS_GMP_LIB
...
VARIABILITY_SOURCES := ODE-SDE/main_variability.cpp  ODE-SDE/GLPKsolve.cpp ODE-SDE/general.cpp
VARIABILITY_LDFLAGS := $(LINK_GLPK_LIB)
VARIABILITY_CPPFLAGS := $(CPPFLAGS) -Wall $(ENABLE_Cxx17)  
VARIABILITY_LD := $(LDPP)

ifdef HAS_GLPK_LIB
  TARGETS += VARIABILITY
  SCRIPTS += VARIABILITY.sh
  VARIABILITY.sh_SOURCEFILE := ODE-SDE/VARIABILITY.sh
endif
