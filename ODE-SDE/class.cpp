       ...
       // #else
        ...
        void  SystEq::SolveSDEEuler(double h,double perc1,double perc2,double Max_Time,int Max_Run,bool Info,double Print_Step,char *argv){
            ...
            void  SystEq::SolveODERK5(double h, double perc1,double Max_Time,bool Info,double Print_Step,char *argv){
                ...
                //A1
                long double ValueA1[nPlaces];
                
                const long double C1d3 = 1.0/3.0;
                const long double C1d6 = 1.0/6.0;
                const long double C1d8 = 1.0/8.0;
                const long double C3d8 = 3.0/8.0; 
                const long double C1d2 = 1.0/2.0; 
                const long double C3d2 = 3.0/2.0; 
                const long double C2d1 =2.0;
                const long double C2d3 =2.0/3.0;
              ...
              /*      //Initialize current step
                ...
                */    
                    ...
                    StepODERK5(ValuePrv,k1);
                    unsigned int i=headDirc;
                    while (i!=DEFAULT)
                    {
                        Value[i]=ValuePrv[i]+k1[i]*h*C1d3;
                        i=VEq[i].getNext();//next place that is computed directly
                    }
                    derived();// it derives all the rest of places
                    ...
                    while (i!=DEFAULT)
                    {
                        Value[i]=ValuePrv[i]+k1[i]*h*C1d6+k2[i]*h*C1d6;
                        i=VEq[i].getNext();//next place that is computed directly
                    }
                    derived();// it derives all the rest of places
                    ...
                    while (i!=DEFAULT)
                    {
                        Value[i]=ValuePrv[i]+k1[i]*h*C1d8+k3[i]*h*C3d8;
                        i=VEq[i].getNext();//next place that is computed directly
                    }
                    derived();// it derives all the rest of places
                    ...
                    while (i!=DEFAULT)
                    {
                        Value[i]=ValuePrv[i]+k1[i]*h*C1d2-k3[i]*h*C3d2+k4[i]*h*C2d1;
                        i=VEq[i].getNext();//next place that is computed directly
                    }
                    derived();// it derives all the rest of places
                    ...
                    while (i!=DEFAULT)
                    {
                        ValueA1[i]=ValuePrv[i]+(C1d2*k1[i]-C3d2*k3[i]+C2d1*k4[i])*h;
                        i=VEq[i].getNext();//next place that is computed directly
                    }
                    derived(ValueA1);// it derives all the rest of places
                     
                
                    i=headDirc;
                    while (i!=DEFAULT)
                    {
                        Value[i]=ValuePrv[i]+(C1d6*k1[i]+C2d3*k4[i]+C1d6*k5[i])*h;
                        if ( Value[i]<0){
#if DEBUG 
                                cout<<"Time: "<<time<<"\nValue place "<<NamePlaces[i]<<" "<<Value[i]<< " Prv:"<<ValuePrv[i]<<endl;
#endif
                                if (Value[i]<perc1){
                                        Value[i]=0;
#if DEBUG                                        
                                        cout<<"Value place "<<NamePlaces[i]<<"is set to"<< Value[i]<<"\n";
                    ...
                    long double error=0.0,tmp_error;
                    for (int i=0;i<nPlaces;i++)
                    {
                        tmp_error= fabs(Value[i]-ValueA1[i])*0.2;
                        if (error<tmp_error)
                            error=tmp_error;
                    }
            
                    //test if the accuracy is reached and update the step if needed 
                    if (error<=perc1){
                        time+=h;
            ...
            /**************************************************************/ 
            void  SystEq::StepODERK5(long double* Yn, long double* Kn){              
