...
then
	...
	${RUN} chown -R user:user ${HOMEDIR}/GreatSPN/SOURCES
	(rm -rf ${HOMEDIR}/GreatSPN/SOURCES/JavaGUI/Editor/build/classes/* )
	(touch ${HOMEDIR}/GreatSPN/SOURCES/JavaGUI/Editor/src/editor/domain/grammar/ExprLang.g4)
	(cd ${HOMEDIR}/GreatSPN/SOURCES && make JavaGUI-antlr)
	# Make everything else
	(cd ${HOMEDIR}/GreatSPN/SOURCES && make)
	echo
	if [ $? -eq 0 ]; then
	...
	(cd ${HOMEDIR}/GreatSPN/SOURCES && ${RUN} make install)
	echo
	if [ $? -eq 0 ]; then
...
cat <<EOF > "${HOMEDIR}/Desktop/Mount Shared Folder"
[Desktop Entry]
Encoding=UTF-8
Type=Application
...
Exec=${HOMEDIR}/GreatSPN/SOURCES/NSRC/VirtualMachineSupport/mount-shared-folder.sh
Comment[en_US]=
Terminal=true
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/Mount Shared Folder"

# Update Deskop link to upgrade GreatSPN
cat <<EOF > "${HOMEDIR}/Desktop/Upgrade GreatSPN"
[Desktop Entry]
Encoding=UTF-8
Type=Application
...
Icon=utilities-terminal-symbolic
Exec=${HOMEDIR}/GreatSPN/SOURCES/NSRC/VirtualMachineSupport/upgrade-greatspn-from-svn.sh
Comment[en_US]=
Terminal=true
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/Upgrade GreatSPN"

# Link to the old GreatSPN interface
cat <<EOF > "${HOMEDIR}/launch-old-greatspn"
#! /bin/sh
export PATH=$PATH:/usr/local/GreatSPN/scripts
greatspn
EOF
${RUN} chown user:user "${HOMEDIR}/launch-old-greatspn"
chmod 755 "${HOMEDIR}/launch-old-greatspn"

cat <<EOF > "${HOMEDIR}/Desktop/GreatSPN-OLD"
[Desktop Entry]
Encoding=UTF-8
Type=Application
Name=GreatSPN-OLD
...
Icon=utilities-terminal-symbolic
Exec=${HOMEDIR}/launch-old-greatspn
Comment[en_US]=
Terminal=true
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/GreatSPN-OLD"

# Link to the new GreatSPN interface
# cat <<EOF > "${HOMEDIR}/Desktop/New GreatSPN Editor"
# EOF
# ${RUN} chown user:user "${HOMEDIR}/Desktop/New GreatSPN Editor"


#-----------------------------------------------------------------
# Documents
cat <<EOF > "${HOMEDIR}/Desktop/GreatSPN VM image README"
=======================================================================================
...
---------------------------------------------------------------------------------------
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/GreatSPN VM image README"

#-----------------------------------------------------------------
cat <<EOF > "${HOMEDIR}/Desktop/HowTo mount shared folder"
...
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/HowTo mount shared folder"

#-----------------------------------------------------------------
cat <<EOF > "${HOMEDIR}/Desktop/HowTo get root access"
The root password is: fedora
EOF
${RUN} chown user:user "${HOMEDIR}/Desktop/HowTo get root access"

# #-----------------------------------------------------------------
# if [[ ! -e ${HOMEDIR}/Desktop/SharedFolder ]] ; then
# 	ln -s /media/vbox_Dati ${HOMEDIR}/Desktop/SharedFolder
# fi
mkdir -p ~/Desktop/SharedFolder

# Remove old upgrade script
# ${RUN} rm -f "${HOMEDIR}/upgrade-greatspn-from-svn"
# ${RUN} rm -f "${HOMEDIR}/Desktop/HowTo update GreatSPN"
